...
var chunkLMW44O4V_cjs = require('./chunk-LMW44O4V.cjs');
...
_Agent_decorators = [chunkGOE646WS_cjs.InstrumentClass({
  prefix: "agent",
...
exports.Agent = class Agent extends (_a = chunkYT4RGZYO_cjs.MastraBase) {
  ...
  constructor(config) {
    ...
    if (config.voice) {
      ...
      this.#voice = new chunkLMW44O4V_cjs.DefaultVoice();
    }
  ...
  async getVoice({
    ...
    if (this.#voice) {
      ...
      return new chunkLMW44O4V_cjs.DefaultVoice();
    }
  }
  get instructions() {
  ...
  get tools() {
    ...
    return chunkZ74LG5VH_cjs.ensureToolProperties(this.#tools);
  }
  getTools({
    runtimeContext = new chunkGPWMM745_cjs.RuntimeContext()
  } = {}) {
    ...
    return resolveMaybePromise(result, tools => {
      ...
      return chunkZ74LG5VH_cjs.ensureToolProperties(tools);
    });
  }
  get llm() {
  ...
  getLLM({
    ...
    return resolveMaybePromise(modelToUse, resolvedModel => {
      const llm = new chunkN3VGOJZV_cjs.MastraLLM({
        model: resolvedModel,
        mastra: this.#mastra
      });
      if (this.#primitives) {
  ...
  async generateTitleFromUserMessage({
    ...
    const normMessage = new chunkDCOKWJ5G_cjs.MessageList().add(message, "user").get.all.ui().at(-1);
    if (!normMessage) {
  ...
  async genTitle(userMessage, runtimeContext, model, instructions) {
    try {
      if (userMessage) {
        const normMessage = new chunkDCOKWJ5G_cjs.MessageList().add(userMessage, "user").get.all.ui().at(-1);
        if (normMessage) {
  ...
  async fetchMemory({
    ...
    userMessages,
    systemMessage,
    messageList = new chunkDCOKWJ5G_cjs.MessageList({
      threadId,
      resourceId
    }),
    runtimeContext = new chunkGPWMM745_cjs.RuntimeContext()
...
`) ?? void 0;
      ...
      const returnList = new chunkDCOKWJ5G_cjs.MessageList().addSystem(systemMessages).add(processedMemoryMessages, "memory").add(newMessages, "user");
      return {
        threadId: thread.id,
  ...
  async getMemoryTools({
    ...
    if (memoryTools) {
      ...
      for (const [toolName, tool] of Object.entries(memoryTools)) {
        ...
        const convertedToCoreTool = chunkZ74LG5VH_cjs.makeCoreTool(toolObj, options);
  ...
  async getAssignedTools({
    ...
    const assignedCoreToolEntries = await Promise.all(assignedToolEntries.map(async ([k, tool]) => {
      ...
      return [k, chunkZ74LG5VH_cjs.makeCoreTool(tool, options)];
    }));
    ...
    if (toolsFromToolsets.length > 0) {
      ...
      for (const toolset of toolsFromToolsets) {
        for (const [toolName, tool] of Object.entries(toolset)) {
          ...
          const convertedToCoreTool = chunkZ74LG5VH_cjs.makeCoreTool(toolObj, options, "toolset");
  ...
  async getClientTools({
    ...
    if (clientToolsForInput.length > 0) {
      ...
      for (const [toolName, tool] of clientToolsForInput) {
        ...
        const convertedToCoreTool = chunkZ74LG5VH_cjs.makeCoreTool(rest, options, "client-tool");
        toolsForRequest[toolName] = convertedToCoreTool;
      }
    }
    return toolsForRequest;
  ...
  async convertTools({
    ...
    if (this.#mastra) {
      mastraProxy = chunkZ74LG5VH_cjs.createMastraProxy({
        mastra: this.#mastra,
        logger
      });
    }
  ...
  __primitive({
    ...
    return {
      before: async () => {
        ...
        const messageList = new chunkDCOKWJ5G_cjs.MessageList({
          threadId,
          resourceId,
        ...
        let [memoryMessages, memorySystemMessage] = await Promise.all([existingThread ? this.getMemoryMessages({
          ...
          vectorMessageSearch: new chunkDCOKWJ5G_cjs.MessageList().add(messages, `user`).getLatestUserContent() || "",
          memoryConfig,
          runtimeContext
...
${(() => {
            let result = ``;
            const messages2 = new chunkDCOKWJ5G_cjs.MessageList().add(resultsFromOtherThreads, "memory").get.all.v1();
            let lastYmd = null;
...
`) ?? void 0;
        ...
        const processedList = new chunkDCOKWJ5G_cjs.MessageList({
          threadId: threadObject.id,
          resourceId,
          generateMessageId: this.#mastra?.generateId?.bind(this.#mastra),
          // @ts-ignore Flag for agent network messages
      ...
      after: async ({
        ...
        const messageListResponses = new chunkDCOKWJ5G_cjs.MessageList({
          threadId,
          resourceId,
          generateMessageId: this.#mastra?.generateId?.bind(this.#mastra),
          // @ts-ignore Flag for agent network messages
  ...
  async prepareLLMOptions(messages, options) {
    ...
    const activeSpan = chunkGOE646WS_cjs.Telemetry.getActiveSpan();
    ...
    if (Object.keys(baggageEntries).length > 0) {
      chunkGOE646WS_cjs.Telemetry.setBaggage(baggageEntries);
    }
    const memory = await this.getMemory({
      runtimeContext
    });
...
//# sourceMappingURL=chunk-R4HT5XUH.cjs.map
//# sourceMappingURL=chunk-R4HT5XUH.cjs.map